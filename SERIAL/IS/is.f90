...
MODULE is_data
  ...
  ! Arrays com indexação 0-based igual ao C original
...
PROGRAM is_serial
  ...
  ! Inicializa arrays de verificação (índice 0-based)
  SELECT CASE (benchmark_class)
    CASE ('S')
      DO i = 0, TEST_ARRAY_SIZE-1
        test_index_array(i) = S_test_index(i+1)
        test_rank_array(i)  = S_test_rank(i+1)
      END DO
    CASE ('W')
      DO i = 0, TEST_ARRAY_SIZE-1
        test_index_array(i) = W_test_index(i+1)
        test_rank_array(i)  = W_test_rank(i+1)
      END DO
    CASE ('A')
      DO i = 0, TEST_ARRAY_SIZE-1
        test_index_array(i) = A_test_index(i+1)
        test_rank_array(i)  = A_test_rank(i+1)
      END DO
    CASE ('B')
      DO i = 0, TEST_ARRAY_SIZE-1
        test_index_array(i) = B_test_index(i+1)
        test_rank_array(i)  = B_test_rank(i+1)
      END DO
    CASE ('C')
      DO i = 0, TEST_ARRAY_SIZE-1
        test_index_array(i) = C_test_index(i+1)
        test_rank_array(i)  = C_test_rank(i+1)
      END DO
  END SELECT

  WRITE(*,*)
  WRITE(*,*) ' NAS Parallel Benchmarks - IS Benchmark (Serial Fortran)'
...
CONTAINS
  ...
  SUBROUTINE create_seq(seed, a)
    ...
    s = seed
    k = MAX_KEY / 4
    ! C: for(i=0; i<NUM_KEYS; i++) key_array[i] = k*x
    DO i = 0, NUM_KEYS-1
      x =  randlc(s, a)
      x = x + randlc(s, a)
      x = x + randlc(s, a)
  ...
  SUBROUTINE rank(iteration)
    ...
    ! C: key_array[iteration] = iteration (0-based)
    key_array(iteration)                  = iteration
    key_array(iteration + MAX_ITERATIONS) = MAX_KEY - iteration

    ! Salva valores de verificação parcial
    DO i = 0, TEST_ARRAY_SIZE-1
    ...
    DO i = 0, NUM_KEYS-1
      bucket_size(ISHFT(key_array(i), -shift)) = &
        bucket_size(ISHFT(key_array(i), -shift)) + 1
    END DO

    ! Ponteiros acumulativos
    bucket_ptrs(0) = 0
    DO i = 1, NUM_BUCKETS-1
      bucket_ptrs(i) = bucket_ptrs(i-1) + bucket_size(i-1)
    END DO

    ! Distribui chaves nos buckets
    ! C: key_buff2[bucket_ptrs[key>>shift]++] = key
    ! O índice usado é o valor ANTES do incremento
    DO i = 0, NUM_KEYS-1
      key = key_array(i)
      bp = ISHFT(key, -shift)
    ...
    ! Conta ocorrências: C: key_buff_ptr[key_buff_ptr2[i]]++
    DO i = 0, NUM_KEYS-1
      key_buff1(key_buff2(i)) = key_buff1(key_buff2(i)) + 1
    END DO

    ! Prefix sum: C: key_buff_ptr[i+1] += key_buff_ptr[i]
    DO i = 0, MAX_KEY-2
      key_buff1(i+1) = key_buff1(i+1) + key_buff1(i)
    END DO

    ! Verificação parcial
    ! C: INT_TYPE key_rank = key_buff_ptr[k-1]
    DO i = 0, TEST_ARRAY_SIZE-1
  ...
  SUBROUTINE full_verify()
    ...
    ! C: key_array[--key_buff_ptr_global[key_buff2[i]]] = key_buff2[i]
    DO i = 0, NUM_KEYS-1
