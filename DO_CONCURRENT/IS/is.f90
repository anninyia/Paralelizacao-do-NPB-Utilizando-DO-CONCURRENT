...
!                  D O   C O N C U R R E N T   F O R T R A N  -  I S     !
...
END MODULE is_utils_dc


PROGRAM is_do_concurrent
  USE is_params_dc
  USE is_data_dc
  USE is_utils_dc
  IMPLICIT NONE
  ...
  WRITE(*,*)
  WRITE(*,*) ' NAS Parallel Benchmarks - IS Benchmark (DO CONCURRENT Fortran)'
  ...
  WRITE(*,*)
  WRITE(*,*) '============================================'
  WRITE(*,*) ' IS Benchmark Completed  [DO CONCURRENT]'
...
CONTAINS
  ...
  SUBROUTINE create_seq(seed, a)
    ...
    s = seed
    k = MAX_KEY / 4
    ! randlc tem estado encadeado — serial obrigatório
    DO i = 0, NUM_KEYS-1
      x =  randlc(s, a)
      x = x + randlc(s, a)
      x = x + randlc(s, a)
  ...
  SUBROUTINE rank(iteration)
    ...
    ! Verificação parcial — independente → DO CONCURRENT
    DO CONCURRENT (i = 0:TEST_ARRAY_SIZE-1)
      partial_verify_vals(i) = key_array(test_index_array(i))
    END DO

    ! Zera bucket_size — independente → DO CONCURRENT
    DO CONCURRENT (i = 0:NUM_BUCKETS-1)
      bucket_size(i) = 0
    END DO

    ! Contagem serial (race condition impossível de resolver sem atomic)
    DO i = 0, NUM_KEYS-1
    ...
    ! Zera key_buff1 — independente → DO CONCURRENT
    DO CONCURRENT (i = 0:MAX_KEY-1)
      key_buff1(i) = 0
    END DO

    ! Histograma serial (race condition)
    DO i = 0, NUM_KEYS-1
  ...
  SUBROUTINE full_verify()
    ...
    ! Verificação — independente → DO CONCURRENT com REDUCE
    j = 0
    DO CONCURRENT (i = 1:NUM_KEYS-1) REDUCE(+:j)
